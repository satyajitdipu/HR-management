...
`                                    <td>
    <div class="dropdown action-label">
        <a id="statusDropdown_{{ $items->id }}" class="btn btn-white btn-sm btn-rounded dropdown-toggle" href="#" data-toggle="dropdown" aria-expanded="false">
            <i class="fa fa-dot-circle-o text-{{ $items->status == 'Active' ? 'success' : 'danger' }}"></i>{{ $items->status }}
        </a>
        <div class="dropdown-menu">
            <a class="dropdown-item" href="#" onclick="updateStatus('{{ $items->id }}', 'Active');"><i class="fa fa-dot-circle-o text-success"></i> Active</a>
            <a class="dropdown-item" href="#" onclick="updateStatus('{{ $items->id }}', 'Inactive');"><i class="fa fa-dot-circle-o text-danger"></i> Inactive</a>
        </div>
    </div>
    <form id="statusForm_{{ $items->id }}" action="{{ route('form/updatedtrainertype') }}" method="POST" style="display: none;">
        @csrf
        <input type="hidden" name="item_id" value="{{ $items->id }}">
        <input type="hidden" name="status" id="statusInput_{{ $items->id }}" value="{{ $items->status }}">
    </form>
</td>
                                    <td class="text-right">
           ...
           $(document).on('click','.edit_type',function()
    {
        ...
        // status
        var status = _this.find(".status").text().trim(); // Trim any leading/trailing whitespace
        $('#e_status').val(status);
    });
        </script>
        {{-- delete model --}}
    ...
    function updateStatus(itemId, status) {
        document.getElementById('statusInput_' + itemId).value = status;
        document.getElementById('statusForm_' + itemId).submit();
    }
</script>
    @endsection
@endsection
