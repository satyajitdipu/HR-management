...
@section('content')
    ...
    <div class="dropdown action-label">
        <a id="statusDropdown" class="btn btn-white btn-sm btn-rounded dropdown-toggle" href="#" data-toggle="dropdown" aria-expanded="false">
            <i class="fa fa-dot-circle-o text-success"></i> {{$appraisal->status}}
        </a>
        <div class="dropdown-menu">
            <a class="dropdown-item" href="#" onclick="updateStatus('active');"><i class="fa fa-dot-circle-o text-success"></i> Active</a>
            <a class="dropdown-item" href="#" onclick="updateStatus('inactive');"><i class="fa fa-dot-circle-o text-danger"></i> Inactive</a>
        </div>
    </div>
</td>

<!-- Hidden input field to store the selected status -->
<input type="hidden" id="statusInput" name="status" value="{{$appraisal->status}}">

<!-- Form to submit the updated status -->
<form id="statusForm" action="{{ route('update/status/active/appraisal') }}" method="POST">
    @csrf
    <!-- Other form fields here -->
    <input type="hidden" id="statusFormInput" name="status">
...
<script>
    function updateStatus(status) {
        ...
        document.getElementById('statusDropdown').innerHTML = `<i class="fa fa-dot-circle-o text-${status === 'active' ? 'success' : 'danger'}"></i> ${status.charAt(0).toUpperCase() + status.slice(1)}`;
