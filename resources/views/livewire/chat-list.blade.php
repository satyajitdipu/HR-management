<div
x-data="{type:'all', selectedConversation: {{ $selectedConversation ?? 'null' }}}"
   x-init="
 ...
 class="flex flex-col transition-all h-full overflow-hidden">
    ...
    @foreach ($users as $user)
        <li class="py-3 hover:bg-gray-50 rounded-2xl dark:hover:bg-gray-700/70 transition-colors duration-150 flex gap-4 relative w-full cursor-pointer px-2"
            wire:click="conversationcreate({{ $user->id }})">
            <a href="#" class="shrink-0">
                <x-avatar src="https://source.unsplash.com/500x500?face-{{$loop->index}}" />
            </a>
