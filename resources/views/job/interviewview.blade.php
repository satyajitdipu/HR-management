        ...
        <div class="content container-fluid" id="app">
            ...
            <div class="row" style="margin-left: -240px;">
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-body">
                            <h4 class="payslip-title">   @if(isset($users->interview))
    @php
    ...
    <div>
        @if(is_array($interview) && count($interview) > 0)
            ...
            @php
                ...
                $endTime = \Carbon\Carbon::parse(trim($times[1]))->format('h:i A');
            @endphp
            {{ $startTime }}
        @endif
    </div>
@endif</h4>
                            <div class="row">
                                <div class="col-sm-6 m-b-20">
                                    @if(!empty($users->user()->get()->first()->avatar))
                                    <img src="{{ URL::to('/assets/images/'. $users->user()->get()->first()->avatar) }}" class="inv-logo" alt="{{ $users->name }}">
                                    @endif
