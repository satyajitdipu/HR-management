                                ...
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        ...
                                        <select class="select select2s-hidden-accessible" style="width: 100%;" tabindex="-1" aria-hidden="true" id="company" name="company" required>
                                            <option value="">-- Select --</option>
                                            @foreach ($linkmanager as $key=>$user )
                                    ...
                                    <thead>
                                     ...
                                     <tbody>
                                        <?php
                                            ...
                                            $key1 = 0;
                                        ?>
                                       @foreach ($permission_lists as $key => $lists )
<tr>
    ...
    <td class="text-center">
        <input type="checkbox" class="option-input checkbox read" id="read_{{ $key }}" name="read[{{ $key }}]" value="Y"{{ $lists->read =="Y" ? 'checked' : ''}}>
        <input type="checkbox" class="option-input checkbox read" id="read_{{ $key }}_n" name="read[{{ $key }}]" value="N" {{ $lists->read =="N" ? 'checked' : ''}}>
    </td>
    <td class="text-center">
        <input type="checkbox" class="option-input checkbox write" id="write_{{ $key }}" name="write[{{ $key }}]" value="Y" {{ $lists->write =="Y" ? 'checked' : ''}}>
        <input type="checkbox" class="option-input checkbox write" id="write_{{ $key }}_n" name="write[{{ $key }}]" value="N" {{ $lists->write =="N" ? 'checked' : ''}}>
    </td>
    <td class="text-center">
        <input type="checkbox" class="option-input checkbox create" id="create_{{ $key }}" name="create[{{ $key }}]" value="Y" {{ $lists->create =="Y" ? 'checked' : ''}}>
        <input type="checkbox" class="option-input checkbox create" id="create_{{ $key }}_n" name="create[{{ $key }}]" value="N" {{ $lists->create =="N" ? 'checked' : ''}}>
    </td>
    <td class="text-center">
        <input type="checkbox" class="option-input checkbox delete" id="delete_{{ $key }}" name="delete[{{ $key }}]" value="Y" {{ $lists->delete =="Y" ? 'checked' : ''}}>
        <input type="checkbox" class="option-input checkbox delete" id="delete_{{ $key }}_n" name="delete[{{ $key }}]" value="N" {{ $lists->delete =="N" ? 'checked' : ''}}>
    </td>
    <td class="text-center">
        <input type="checkbox" class="option-input checkbox import" id="import_{{ $key }}" name="import[{{ $key }}]" value="Y" {{ $lists->import =="Y" ? 'checked' : ''}}>
        <input type="checkbox" class="option-input checkbox import" id="import_{{ $key }}_n" name="import[{{ $key }}]" value="N" {{ $lists->import =="N" ? 'checked' : ''}}>
    </td>
    <td class="text-center">
        <input type="checkbox" class="option-input checkbox export" id="export_{{ $key }}" name="export[{{ $key }}]" value="Y" {{ $lists->export =="Y" ? 'checked' : ''}}>
        <input type="checkbox" class="option-input checkbox export" id="export_{{ $key }}_n" name="export[{{ $key }}]" value="N" {{ $lists->export =="N" ? 'checked' : ''}}>
    </td>
</tr>
@endforeach
