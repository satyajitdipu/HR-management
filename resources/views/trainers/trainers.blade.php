...
@section('content')
    ...
    <tr>
        ...
        <td>
    <form id="statusForm_{{ $trainer->id }}" action="{{ route('update-status') }}" method="POST">
        @csrf
        <!-- Hidden input for the trainer's ID -->
        <input type="hidden" name="trainer_id" value="{{ $trainer->id }}">
        <!-- Hidden input for the trainer's initial status -->
        <input type="hidden" name="initial_status" value="{{ $trainer->status }}">
        <!-- Hidden input for the selected status -->
        <input type="hidden" id="selectedStatus_{{ $trainer->id }}" name="status" value="{{ $trainer->status }}">
        <!-- Display the trainer's current status -->
        <div class="dropdown action-label">
            <a id="statusDropdown_{{ $trainer->id }}" class="btn btn-white btn-sm btn-rounded dropdown-toggle" href="#" data-toggle="dropdown" aria-expanded="false">
                <i class="fa fa-dot-circle-o text-success"></i>{{ $trainer->status }}
            </a>
            <div class="dropdown-menu">
                <a class="dropdown-item" href="#" onclick="updateStatus('{{ $trainer->id }}', 'Active');"><i class="fa fa-dot-circle-o text-success"></i> Active</a>
                <a class="dropdown-item" href="#" onclick="updateStatus('{{ $trainer->id }}', 'Inactive');"><i class="fa fa-dot-circle-o text-danger"></i> Inactive</a>
            </div>
        </div>
        <button type="submit" id="submitForm_{{ $trainer->id }}" style="display: none;">Submit</button>
    </form>
</td>
    ...
    <script>
    function updateStatus(trainerId, status) {
        // Update the dropdown text
        let dropdownElement = document.getElementById('statusDropdown_' + trainerId);
        dropdownElement.innerHTML = `<i class="fa fa-dot-circle-o text-${status === 'Active' ? 'success' : 'danger'}"></i> ${status}`;

        // Update the selected status input field
        let selectedStatusElement = document.getElementById('selectedStatus_' + trainerId);
        selectedStatusElement.value = status;

        // Submit the form
        let submitButton = document.getElementById('submitForm_' + trainerId);
        submitButton.click();
    }
</script>
