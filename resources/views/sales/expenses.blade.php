...
@section('content')
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <h2 class="table-avatar">
                                            <a href="profile.html" class="avatar avatar-xs"><img src="{{ URL::to('/storage/documents/'.$item->attachments) }}" alt=""></a>
    ...
    <div class="dropdown action-label">
        ...
        <div class="dropdown-menu dropdown-menu-right">
            <form id="updateStatusForm{{ $item->id }}" action="{{ route('updateExpense') }}" method="POST">
                @csrf
                <input type="hidden" name="id" value="{{ $item->id }}">
                <input type="hidden" name="status" value="">
                <button type="submit" class="dropdown-item" onclick="setStatus('{{ $item->id }}', 'Pending'); return false;"><i class="fa fa-dot-circle-o text-danger"></i> Pending</button>
                <button type="submit" class="dropdown-item" onclick="setStatus('{{ $item->id }}', 'Approved'); return false;"><i class="fa fa-dot-circle-o text-success"></i> Approved</button>
            </form>
        </div>
    </div>
</td>
                                    <td class="text-right">
                                        <div class="dropdown dropdown-action">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right">
                                                ...
                                                <a class="dropdown-item delete_expense" href="#" data-toggle="modal" data-target="#delete_expense" data-id="{{ $item->id }}" data-attachments="{{ $item->attachments }}"><i class="fa fa-trash-o m-r-5"></i> Delete</a>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
    ...
    function setStatus(itemId, status) {
        const form = document.querySelector(`#updateStatusForm${itemId}`);
...
<script>
    ...
    $('#delete_expense').find('.e_id').val(id);
    $('#delete_expense').find('.d_attachments').val(attachments);
});
